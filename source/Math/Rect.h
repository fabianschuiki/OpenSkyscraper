...
public:
	...
	inline Vector2D<T> point(Point p) const {
		switch (p) {
			case kMinXminY:	return minXminY(); break;
			case kMinXmidY:	return minXmidY(); break;
			case kMinXmaxY:	return minXmaxY(); break;
				
			case kMidXminY:	return midXminY(); break;
			case kMidXmaxY:	return midXmaxY(); break;
				
			case kMaxXminY:	return maxXminY(); break;
			case kMaxXmidY:	return maxXmidY(); break;
			case kMaxXmaxY:	return maxXmaxY(); break;
		};
		return center();
	}
	...
	inline T distanceX(const Rect<T> &r) const {
		if (maxX() >= r.minX() && minX() <= r.maxX())
			return 0;
		return std::min<T>(	fabs((float)r.minX() - maxX()),
							fabs((float)r.maxX() - minX()));
	}
